...
public class ProductosTableModel extends AbstractTableModel {
    ...
    public ProductosTableModel() {
        lis = ProductoDatas.list("");
    }

    public ProductosTableModel(String filter) {
        lis = ProductoDatas.list(filter);
    }
    
    public ProductosTableModel(String filter, Date fi, Date ff) {
        lis = ProductoDatas.list(filter, fi, ff);
    }
    

    @Override
    public Object getValueAt(int row, int column) {
        ProductoProfe d = (ProductoProfe) lis.get(row);
        switch (column) {
            case 0:
                return row + 1;
            case 1:
    ...
    public void addRow(ProductoProfe d) { // con db no se usa
        this.lis.add(d);
